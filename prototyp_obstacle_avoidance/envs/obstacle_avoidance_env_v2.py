"""
ObstacleAvoidanceEnvV2: reward-reworked variant of ObstacleAvoidanceEnv.
...
Changes from v1:
  ...
  - Obstacle proximity penalty kept unchanged.

Reward components:
  progress            -- positive when drone moves toward target
  close               -- exp(-dist): peaks at target, strong gradient within ~2 m
  obstacle_proximity  -- linear penalty within safety_radius of any obstacle
  crash               -- one-time penalty on crash (incl. obstacle collision)
  success             -- one-time bonus on successful hover

Everything else (obs, actions, PID, obstacles, depth camera, reset, termination)
is identical to v1.
"""
...
class ObstacleAvoidanceEnvV2:
    ...
    def build(self, pre_build_cameras=None):
        ...
        print(f"[ObstacleAvoidanceEnvV2] Renderer: {renderer_name} (cfg={cfg_val!r})")
        ...
        if self._using_batch_renderer:
            ...
            # Then tilt 45 deg downward: compose with R_x(-45 deg) in camera frame
            # Result: look direction = body [1, 0, -1]/sqrt(2) (forward + 45 deg down)
    ...
    def _render_depth_rasterizer(self):
        ...
        plane (horizontal forward), tilted 45 deg downward.
        ...
        for env_i in range(self.num_envs):
            ...
            # 45 deg downward tilt: equal forward + down components
            cam_lookat = (
    ...
    def _compute_obs(self) -> TensorDict:
        ...
        # State vector (17-dim, identical to v1)
