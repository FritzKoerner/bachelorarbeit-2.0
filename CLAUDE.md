...
- **prototyp_corridor_navigation/** — Forked from obstacle_avoidance env v2. Fixed-axis corridor slalom along +X with mixed-shape obstacles (1 box, 1 sphere, 1 cylinder, 1 pillar cylinder) and physical boundary walls + ceiling that the depth camera perceives. No v1/v2 split, no strategic/vineyard placers.
...
**Corridor geometry (metres)**: X `[0.0, 16.0]`, Y `[-4.0, 4.0]`, Z `[0.0, 6.0]`. Spawn uniform in X `[0.5, 2.5]` × Y `[-2.5, 2.5]` at Z `4.0`. Target fixed at `(15.0, 0.0, 1.0)`. Left/right walls at y=±4.1 and ceiling at z=6.1 are **physical box entities** (indices 4-6 in `self.obstacles`, flagged `is_wall=True`) visible to the depth camera; ground plane at z=0 handles the floor. Drone crashes on wall-contact (collision check at radius 0.3 m) at y=±3.7 / z=5.7 before reaching the numeric bounds; `out_of_corridor` termination is retained as a safety net and still logged separately.

**Obstacle mix (7 total: 4 random + 3 walls)**: random obstacles are 1 Box, 1 Sphere, 1 Cylinder, 1 thin-tall pillar Cylinder, shape-interleaved so slice 0 gets (Box+Sphere) and slice 1 gets (Cyl+Pillar). Sizes configurable via `env_cfg` keys `corridor_{box_sizes, sphere_radii, cylinder_specs, pillar_specs}` — all four must have the same length, and `4 * len(box_sizes)` must equal `2 * len(slice_centres)` (two obstacles per slice); an assertion checks this at init. Placed in 2 X-slices (centres `[5.0, 10.0]`) as **pair-straddling slaloms**: per slice/env the placement draws axis ∈ {Y, Z} (forced to Y when the pair contains a pillar), side ∈ {-1, +1}, distance `d ∈ [0, 3]` m from the spawn→target line, and face-to-face gap ∈ `[1, 2]` m. Formula: `A_along = line_along + side·d`, `B_along = A_along − side·(A_half + gap + B_half)`, so the face-to-face gap between the pair is always exactly `gap` regardless of shape sizes. Walls are fixed (never moved by the placement loop or curriculum).

**Shape-aware collision**: `_compute_obstacle_distances()` computes per-shape distances (box AABB / sphere radial / cylinder radial+axial) for every obstacle and dispatches via `torch.where` on `self.obstacle_shape_type`. Fully vectorised across the `n_obs` axis (now 7) — no Python loop per obstacle. Walls participate like any other box obstacle.

**Reward scales**: `progress +0.5`, `close +0.1`, `obstacle_proximity -0.6` (safety_radius 1.5 m, applies to all 7 obstacles including walls/ceiling), `crash -10.0`, `success +20.0`, `out_of_corridor -10.0`. Proximity penalty was tuned down (was 3.0 m) so walls don't flood the signal — at spawn (z=4, |y|≤2.5), the drone is ≥1.5 m from every wall/ceiling surface, so spawn itself triggers zero proximity penalty.

**Curriculum**: random obstacles (indices 0-3) stay underground (`z = -100`) until `global_step >= curriculum_steps`; walls and ceiling (indices 4-6, `is_wall=True`) remain in place from step 0 so the drone learns corridor boundaries visually during warm-up. Default `curriculum_steps = 6020` (301 iterations × num_steps_per_env=20). The `out_of_corridor` termination is also enforced from step 0 as a safety net.
